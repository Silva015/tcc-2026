...
In stark contrast, the proposed TransferKAN model achieved its 0.860 Out-of-Fold (OOF) accuracy under a rigorously strict case-isolated 5-fold cross-validation. When comparing the models under the same rigorous K-fold evaluation paradigm (0.860 for TransferKAN vs. a median of $\sim$0.73 for ResNet152), it becomes unequivocally clear that TransferKAN is vastly superior and significantly more stable than the baseline. Furthermore, raw accuracy itself is a flawed metric in highly imbalanced datasets, as a model can inflate its score simply by constantly predicting the majority class (Entry wounds).
